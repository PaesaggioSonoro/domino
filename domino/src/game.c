...
/**
 * @def INACTIVE_TIME
 * @brief Frames needed to consider player "inactive"
 */
/**
 * @def MESSAGE(a)
 * @brief Set an info message
 */
/**
 * @def CLEAR_MESSAGE
 * @brief Clear the info message
 */
/**
 * @def INACTIVE_MESSAGE(a, t)
 * @brief Set an info message if player is inactive
 */
/**
 * @def INACTIVE_MESSAGE2(a, t)
 * @brief Set a second info message if player is still inactive
 */

/**
 * @brief When changing status, the next frame will be this status
 */
int nextStatus = GameStatus_NULL;

// MENU
bool MenuSinglePlayerSelected = false; /// True if the single player mode is selected in the menu
bool MenuWithAISelected = false; /// True if the multiplayer mode is selected in the menu

// GAME
bool SelectingFromTable = false; /// True if the player is selecting a card from the table
bool playerTurn = true; /// True if it's the player's turn
int inactiveTimer = 0; /// Timer for user inactivity and consequent help messages
int AIinactiveTimer = 0; /// Timer for AI "think" delays
Card * AImove = NULL; /// Card that the AI is moving
...
/**
 * @brief True if the game is waiting for the next player input
 */
bool waitingNextInput = false;

/**
 * @brief Main game loop
 * @details This function is the main game loop, it handles the game status and the game logic
 */
void GameLoop(){
